...
void PhotonIntegrator::Preprocess(const Scene *scene) {
    ...
    while (!causticDone || !directDone || !indirectDone) {
        ...
        float u[4];
        u[0] = (float)RadicalInverse(nshot + 1, 2);
        u[1] = (float)RadicalInverse(nshot + 1, 3);
        ...
        if (!alpha.Black()) {
            ...
            while (scene->Intersect(photonRay, &photonIsect)) {
                ...
                if (hasNonSpecular) {
                    ...
                    Photon photon(photonIsect.dg.p, alpha.ToXYZ(), wo);
                    if (nIntersections == 1) {
                ...
                float u1, u2, u3;
                if (nIntersections == 1) {
                    u1 = (float)RadicalInverse(nshot+1, 13);
                    u2 = (float)RadicalInverse(nshot+1, 17);
                    u3 = (float)RadicalInverse(nshot+1, 19);
                }
                else {
        ...
        if (finalGather) {
            ...
            for (int i = 0; i < gatherSamples; ++i) {
                ...
                if (scene->Intersect(bounceRay, &gatherIsect)) {
                    ...
                    BSDF *gatherBSDF = gatherIsect.GetBSDF(bounceRay, fabsf(2.f * u3 - 1.f));
