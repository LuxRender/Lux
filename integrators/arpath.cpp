...
void ARPathIntegrator::Preprocess(const TsPack *tspack, const Scene *scene)
{
...
u_int ARPathIntegrator::Li(const TsPack *tspack, const Scene *scene,
	const Sample *sample) const
{
	...
	for (u_int pathLength = 0; ; ++pathLength) {
		...
		SWCSpectrum Lv;
		const u_int g = scene->volumeIntegrator->Li(tspack, scene, ray, sample, &Lv, &alpha, from_IsSup, path_type); 
		if (!Lv.Black()) {
			Lv *= pathThroughput;
			L[g] += Lv;
			V[g] += Lv.Filter(tspack) * VContrib;
...
SurfaceIntegrator* ARPathIntegrator::CreateSurfaceIntegrator(const ParamSet &params)
{
	...
	ARPathIntegrator *pi = new ARPathIntegrator(rstrategy, max(maxDepth, 0), RRcontinueProb, include_environment);
