...
public:
	...
	inline void Sample(float u1, float u2) {
		...
		for (u_int i = 0; i < WAVELENGTH_SAMPLES; ++i) {
			...
			spect_w.c[i] = (double) spd_w->sample(waveln);
			spect_c.c[i] = (double) spd_c->sample(waveln);
			spect_m.c[i] = (double) spd_m->sample(waveln);
			spect_y.c[i] = (double) spd_y->sample(waveln);
			spect_r.c[i] = (double) spd_r->sample(waveln);
			spect_g.c[i] = (double) spd_g->sample(waveln);
			spect_b.c[i] = (double) spd_b->sample(waveln);
