...
class BlenderMusgraveTexture3D : public Texture<T> {
public:
...
template <class T> Texture<Spectrum> *BlenderMusgraveTexture3D<T>::CreateSpectrumTexture(
        const Transform &tex2world,
        const TextureParams &tp) {
    // Initialize 3D texture mapping _map_ from _tp_
    TextureMapping3D *map = new IdentityMapping3D(tex2world);
    ...
    return new BlenderMusgraveTexture3D<Spectrum>(
			tex1,
			tex2,
            tp.FindFloat("h", 1.0f),
            tp.FindFloat("lacu", 2.0f),
