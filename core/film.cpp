	...
	const CameraResponse *response, float dither)
{
	...
	if (bloomRadius > 0.f && bloomWeight > 0.f) {
		if(bloomUpdate) {
			...
			BloomFilter bf(xResolution, yResolution, bloomWidth, bloomFilter, bloomImage, xyzpixels);
			tbb::parallel_for(tbb::blocked_range2d<u_int>(0, yResolution, 0, xResolution), bf);
