...
def exportMaterial(mat):
	...
	def write_color_imagemap( chan, name, filename ):
		return "Texture \"%s-%s\" \"color\" \"imagemap\" \"string filename\" [\"%s\"] \"float vscale\" [-1.0]\n" %(chan, name, filename)

	def write_float_constant( chan, name, v ):
		return "Texture \"%s-%s\" \"float\" \"constant\" \"float value\" [%f]\n" %(chan, name, v)

	def write_float_imagemap( chan, name, filename ):
		return "Texture \"%s-%s\" \"float\" \"imagemap\" \"string filename\" [\"%s\"] \"float vscale\" [-1.0]\n" %(chan, name, filename)
	...
	def write_color_param( m, param, chan, r, g, b ):
		...
		if (chan_file == 'NONE'):
			...
			return write_color_imagemap( param, m.name, chan_file )
	...
	if (mat_type == 100):
		...
		str += "# Type: emitter\n"
		str += write_color_param( mat, "L", 'EMIT', mat.R, mat.G, mat.B ) # TODO tune emission
		...
		str += write_color_param( mat, "Ks", 'SPEC', mat.specR, mat.specG, mat.specB )
...
def exportMaterialGeomTag(mat):
	mat_type = getMaterialType(mat)
	if (mat_type == 100):
		str = "AreaLightSource \"area\" \"integer nsamples\" [1] \"texture L\" \"L-%s\"" %mat.name
	else:
...
def save_lux(filename, unindexedname):
	...
	file.write("	 	\"integer hdr_writeinterval\" [320]\n")
	file.write("	 \"string ldr_filename\" [\"out.tga\"]\n")
	file.write("		\"integer ldr_writeinterval\" [240]\n")
	file.write("		\"integer ldr_displayinterval\" [10]\n")
	file.write("		\"string tonemapper\" [\"reinhard\"]\n")
	file.write("			\"float reinhard_prescale\" [1.0]\n")
	file.write("			\"float reinhard_postscale\" [1.0]\n")
	file.write("			\"float reinhard_burn\" [6.0]\n")
	file.write("		\"float gamma\" [2.2]\n")
	file.write("		\"float dither\" [0.5]\n\n")

	file.write("PixelFilter \"gaussian\" \"float xwidth\" [2.000000] \"float ywidth\" [2.000000]\n")
	file.write("\n")

	##### Write Pixel Sampler ######
	file.write("Sampler \"lowdiscrepancy\" \"bool progressive\" [\"true\"]\n")
	file.write("\n")

	##### Write Integrator ######
	file.write("SurfaceIntegrator \"path\" \"integer maxdepth\" [1000]\n")
	file.write("\n")
...
####################################################################
	...
	global SaveUTMExr, SaveTMExr, ToneMapType, ToneMapScale, Autoexp, ToneMapPreScale, ToneMapPostScale, ToneMapBurn
...
########################################
	...
	global SaveUTMExr, SaveTMExr, ColExponent
...
#######################################
	...
	#	ToneMapBurn = Draw.Number("Burn: ", evtNoEvt, 95, 150, 135, 18, ToneMapBurn.val, 0.0, 1.0, "1.0: no burn out, 0.1 lot of burn out")
...
##################
	...
	BGL.glColor3f(0.9,0.9,0.9);BGL.glRasterPos2i(10,205) ; Draw.Text("LuxBlend v0.1alpha7", "small")
