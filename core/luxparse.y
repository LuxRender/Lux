...
%token NAMEDMATERIAL OBJECTBEGIN OBJECTEND OBJECTINSTANCE MOTIONINSTANCE
%token LIGHTGROUP PIXELFILTER REVERSEORIENTATION ROTATE SAMPLER SCALE
%token SEARCHPATH PORTALSHAPE SHAPE SURFACEINTEGRATOR TEXTURE TRANSFORMBEGIN
%token TRANSFORMEND TRANSFORM TRANSLATE VOLUME VOLUMEINTEGRATOR WORLDBEGIN
%token WORLDEND
...
| EXTERIOR STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CP);
	Context::GetActive()->Exterior($2, params);
	FreeArgs();
}
...
| INTERIOR STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CP);
	Context::GetActive()->Interior($2, params);
	FreeArgs();
}
