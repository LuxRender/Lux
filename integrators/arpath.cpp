...
void ARPathIntegrator::Preprocess(const RandomGenerator &rng, const Scene &scene)
{
...
u_int ARPathIntegrator::Li(const Scene &scene, const Sample &sample) const
{
	u_int nrContribs = 0;
...
//------------------------------------------------------------------------------
// DataParallel integrator ARPathState code
//------------------------------------------------------------------------------
...
bool ARPathState::Init(const Scene &scene) {
	flags = 0;
...
void ARPathState::Free(const Scene &scene) {
	delete[] L;
	delete[] V;
	delete[] Ld;
	delete[] Vd;
...
void ARPathState::Terminate(const Scene &scene, const u_int bufferId,
		const float alpha) {
	const u_int lightGroupCount = scene.lightGroups.size();
	for (u_int i = 0; i < lightGroupCount; ++i) {
		if (!L[i].Black())
...
bool ARPathState::TerminatePath(const Scene &scene, const u_int bufferId,
			const float alpha) {
	// Check I if I have still to trace last direct light rays
	if (tracedShadowRayCount > 0) {
		SetTerminate();
		SetState(ARPathState::CONTINUE_SHADOWRAY);
		return false;
	} else {
...
//------------------------------------------------------------------------------
// DataParallel integrator ARPathIntegrator code
//------------------------------------------------------------------------------
...
bool ARPathIntegrator::GenerateRays(const Scene &,
	...
	switch (pathState->GetState()) {
		...
		default:
			throw std::runtime_error("Internal error in ARPathIntegrator::GenerateRays(): unknown path state.");
	}
...
SurfaceIntegrator* ARPathIntegrator::CreateSurfaceIntegrator(const ParamSet &params)
{
	// general
	...
	ARPathIntegrator *pi = new ARPathIntegrator(rstrategy, max(maxDepth, 0), RRcontinueProb,
...
static DynamicLoader::RegisterSurfaceIntegrator<ARPathIntegrator> r("arpath");
