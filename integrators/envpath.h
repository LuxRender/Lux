...
public:
    ...
    ENVPathIntegrator(RRStrategy rst, u_int md, float cp, bool ie, bool dls, bool rflm) : SurfaceIntegrator(),
		bufferId(0), hints(), rrStrategy(rst), maxDepth(md), continueProbability(cp),
		sampleOffset(0), includeEnvironment(ie), enableDirectLightSampling(dls), ReflectionMethod(rflm) {
		AddStringConstant(*this, "name", "Name of current surface integrator", "envpath");
		AddIntAttribute(*this, "maxDepth", "Path max. depth", &ENVPathIntegrator::GetMaxDepth);
	}
...
private:
	...
	bool includeEnvironment, enableDirectLightSampling, ReflectionMethod;
};
