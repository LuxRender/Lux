#!/usr/bin/python
#! This script generates a build number for Lux and it updates the 
#! header file passe as a parameter.
#!
...
if isOK:
  bnRE = re.compile(r"^#define\s+LUX_VN_BUILD\s+(\d+)", re.MULTILINE)

  replacement = "#define LUX_VN_BUILD %s\n" % buildNo
