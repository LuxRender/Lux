...
SWCSpectrum BidirIntegrator::Li(const Scene *scene,
...
int BidirIntegrator::generatePath(const Scene *scene, const Ray &r,
	...
	while (nVerts < maxVerts) {
		...
		SWCSpectrum fr = v.bsdf->Sample_f(v.wi, &v.wo, u1, u2, u3,
...
SWCSpectrum BidirIntegrator::evalPath(const Scene *scene, BidirVertex *eye, int nEye,
		BidirVertex *light, int nLight) const {
	SWCSpectrum L(1.);
