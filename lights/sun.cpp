...
public:
	...
	SWCSpectrum f(const TsPack *tspack, const Vector &wo, const Vector &wi) const {return (wo.z < 0.f || wi.z < 0.f || (wo.x * wo.x + wo.y * wo.y) > sin2ThetaMax || (wi.x * wi.x + wi.y * wi.y) > sin2ThetaMax) ? 0.f : 1.f;}
