...
HitPoints::HitPoints(HybridSPPMRenderer *engine) {
	...
	for (u_int i = 0; i < (*hitPoints).size(); ++i) {
		...
		hp->photonCount = 0;
		hp->reflectedFlux = SWCSpectrum();

		// hp->accumPhotonRadius2 is initialized in the Init() method
		hp->accumPhotonCount = 0;
		hp->accumReflectedFlux = SWCSpectrum();
		hp->accumDirectLightRadiance = SWCSpectrum();

		hp->accumRadiance = SWCSpectrum();
		hp->constantHitsCount = 0;
		hp->surfaceHitsCount = 0;
		hp->radiance = SWCSpectrum();
	}
}
...
void HitPoints::AccumulateFlux(const unsigned long long photonTraced,
	...
	const u_int workSize = hitPoints->size() / count;
	const u_int first = workSize * index;
	const u_int last = (index == count - 1) ? hitPoints->size() : (first + workSize);
	assert (first >= 0);
...
void HitPoints::SetHitPoints(RandomGenerator *rndGen,
	...
	const u_int workSize = pixelCount / count;
	const u_int first = workSize * index;
	const u_int last = (index == count - 1) ? pixelCount : (first + workSize);
	assert (first >= 0);
	...
	while(todoEyePathCount > 0) {
		...
		if (rayBuffer->GetRayCount() > 0) {
			...
			for (u_int i = 0; i < rayBuffer->GetRayCount(); ++i) {
				EyePath *eyePath = *todoEyePathsIterator;
...
void HitPoints::UpdatePointsInformation() {
	...
	for (u_int i = 0; i < (*hitPoints).size(); ++i) {
		...
		if (hp->type == SURFACE) {
			...
			maxPhotonRaidus2 = luxrays::Max(maxPhotonRaidus2, hp->accumPhotonRadius2);
		}
	}
