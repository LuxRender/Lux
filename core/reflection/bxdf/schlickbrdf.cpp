...
SchlickGlossyBRDF::SchlickGlossyBRDF(const Fresnel *fr, float r, float p, bool mb)
	: BxDF(BxDFType(BSDF_REFLECTION | BSDF_GLOSSY)),
	fresnel(fr), roughness(r), anisotropy(p), multibounce(mb)
{
}
...
bool SchlickGlossyBRDF::SampleF(const SpectrumWavelengths &sw, const Vector &wo,
	Vector *wi, float u1, float u2, SWCSpectrum *const f_, float *pdf, 
	float *pdfBack, bool reverse) const
{
	// No sampling on the back face
	...
	*wi = 2.f * cosWH * H - wo;
	...
	const float specPdf = SchlickZ(H.z) * SchlickA(H) /
		(4.f * M_PI);
	*pdf = specPdf / fabsf(wo.z);
	if (!(*pdf > 0.f))
		return false;
	if (pdfBack)
		*pdfBack = specPdf / fabsf(wi->z);
